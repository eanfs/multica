...
| `packages/nextjs` | `core/`、`next/*` | `react-router-dom`、electron、其他 app |
| `apps/web/platform/` | `next/*` | 其他 app |
| `apps/aurora/platform/` | `next/*` | 其他 app |
...
两个 app 都有的逻辑，**必须**抽到共享包。"小段重复"也不算例外。Web 和 Aurora 共享与框架无关的逻辑走 `packages/core`，依赖 `next/*` 的辅助函数走 `packages/nextjs`。Mobile 拥有独立的 UI、数据层和发布流程，只共享类型与纯函数。
