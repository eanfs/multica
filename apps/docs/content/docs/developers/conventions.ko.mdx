...
| `packages/nextjs` | `core/`, `next/*` | `react-router-dom`, electron, 다른 앱 |
| `apps/web/platform/` | `next/*` | 다른 앱 |
| `apps/aurora/platform/` | `next/*` | 다른 앱 |
...
두 앱 모두에 동일한 로직이 나타난다면 반드시 공유 패키지로 추출해야 합니다. "사소한" 중복이라는 예외는 없습니다. Web과 Aurora는 프레임워크에 종속되지 않는 로직은 `packages/core`에서, `next/*`에 의존하는 헬퍼는 `packages/nextjs`에서 공유합니다. Mobile은 독립된 UI, 데이터 계층, 출시 절차를 가지며 유형과 순수 함수만 공유합니다.
