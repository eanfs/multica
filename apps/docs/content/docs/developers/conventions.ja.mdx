...
| `packages/nextjs` | `core/`, `next/*` | `react-router-dom`, electron, 他のアプリ |
| `apps/web/platform/` | `next/*` | 他のアプリ |
| `apps/aurora/platform/` | `next/*` | 他のアプリ |
...
両方のアプリに同じロジックが現れる場合は、必ず共有パッケージに抽出しなければなりません。「ささいな」重複という例外はありません。Web と Aurora は、フレームワーク非依存のロジックを `packages/core`、`next/*` に依存するヘルパーを `packages/nextjs` で共有します。Mobile は独自の UI、データ層、リリースフローを持ち、型と純粋関数だけを共有します。
