...
| `packages/nextjs` | `core/`, `next/*` | `react-router-dom`, electron, other apps |
| `apps/web/platform/` | `next/*` | other apps |
| `apps/aurora/platform/` | `next/*` | other apps |
...
If logic appears in both apps, it MUST be extracted to a shared package. There are no exceptions for "small" duplication. Web and Aurora share framework-agnostic logic through `packages/core` and helpers built on `next/*` through `packages/nextjs`. Mobile owns a separate UI, data layer, and release process; it shares only types and pure functions.
