...
- **Plan safety is half done.** Its content moderation tasks (#63, #65) are implemented: a `Moderator` adapter with a bilingual keyword blocklist and local image screening, wired into generation create (prompt pre-screen) and artifact writeback (post-screen, migrations `510`–`511`). Its entitlement gates (`GateAurora*`) stay Deferred — self-hosted `entitlement` fails open without a cloud BaseURL, so the MVP runs without them.
- Not started, not yet split into scrum tickets: Plan 5 (订阅 + Stripe). Together with the Plan safety gates it is the remaining piece before the spec's "sellable" milestone. Renumber its migrations before starting — the plan assumes `459` but the repository is past `509`.
